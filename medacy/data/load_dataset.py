...
def load_END():
    ...
    """
    package_name = 'medacy.data'
    resource_directory = 'end'
    contents = resource_listdir(package_name, resource_directory)
    entities = resource_string(package_name, resource_directory+'/END.types').decode('utf-8').split("\n")
    files = [file[:-4] for file in contents if file.endswith('.ann')]
    files = ((file_name,
              resource_string(package_name, resource_directory+"/%s.txt" % file_name).decode('utf-8'),
              resource_string(package_name, resource_directory + "/%s.ann" % file_name).decode('utf-8')
              )
